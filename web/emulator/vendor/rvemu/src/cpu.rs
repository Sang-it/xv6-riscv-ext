...
impl Cpu {
    ...
    pub fn check_pending_interrupt(&mut self) -> Option<Interrupt> {
        // WFI must resume for a locally enabled pending interrupt even when
        // global SIE/MIE is clear. Current xv6 intentionally does intr_off()
        // before WFI to avoid losing a wake-up in its idle scheduler.
        if self.state.read(MIE) & self.state.read(MIP) != 0 {
            self.idle = false;
        }
    ...
    pub fn fetch(&mut self, size: u8) -> Result<u64, Exception> {
        ...
        // A 32-bit instruction may start at the last halfword of a virtual
        // page when compressed instructions are enabled. The next virtual
        // page need not be adjacent in physical RAM (c4 hits this case).
        if size == WORD && self.pc & (PAGE_SIZE - 1) == PAGE_SIZE - 2 {
            let next = self.translate(self.pc.wrapping_add(2), AccessType::Instruction)?;
            let low = self.bus.read(p_pc, HALFWORD).map_err(|_| Exception::InstructionAccessFault)?;
            let high = self.bus.read(next, HALFWORD).map_err(|_| Exception::InstructionAccessFault)?;
            return Ok(low | (high << 16));
        }
...
#[cfg(test)]
mod browser_regressions {
    use super::*;

    #[test]
    fn fetch_instruction_across_noncontiguous_pages() {
        let mut cpu = Cpu::new();
        let base = crate::bus::DRAM_BASE;
        let pointer = |addr: u64| ((addr >> 12) << 10) | 1u64;
        cpu.bus.write(base, pointer(base + 0x1000), DOUBLEWORD).unwrap();
        cpu.bus.write(base + 0x1000, pointer(base + 0x2000), DOUBLEWORD).unwrap();
        cpu.bus.write(base + 0x2008, pointer(base + 0x10000) | 0xa, DOUBLEWORD).unwrap();
        cpu.bus.write(base + 0x2010, pointer(base + 0x20000) | 0xa, DOUBLEWORD).unwrap();
        cpu.bus.write(base + 0x10ffe, 0xe0ef, HALFWORD).unwrap();
        cpu.bus.write(base + 0x20000, 0x802f, HALFWORD).unwrap();
        cpu.mode = Mode::Supervisor;
        cpu.page_table = base;
        cpu.enable_paging = true;
        cpu.pc = 0x1ffe;
        assert_eq!(cpu.fetch(WORD).unwrap(), 0x802fe0ef);
        cpu.execute().unwrap();
        assert_eq!(cpu.pc, 0);
        assert_eq!(cpu.xregs.read(1), 0x2002);
    }

    #[test]
    fn pending_timer_wakes_wfi_with_global_interrupts_disabled() {
        let mut cpu = Cpu::new();
        cpu.mode = Mode::Supervisor;
        cpu.idle = true;
        cpu.state.write(MIE, STIP_BIT);
        cpu.state.write(MIP, STIP_BIT);
